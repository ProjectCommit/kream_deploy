$(function(){
	$.ajax({
		type: 'post',
		url: '/kream/ranking/rankingListMaleshoes',
		success: function(data){
			...
			$.each(data,function(index,item){
				...
				$('.maleshoes').append(result);	
				}					 
			});


			//START 선택적 더보기 기능
			var classArray = ["maleshoes1"];
			var parent;
			...
			$(document).on('click', '.wishButton', function () {
				var productId = $(this).closest('.itemdivide').find('.productNum').text();
				console.log(productId+"productIdproductIdproductIdproductIdproductId");
				$.ajax({
					type: 'post',
					url: '/kream/ranking/addWishLists',
					data: {
							productId: productId,
							email: "1"
						},
					success: function(data){
						...
						window.location.href='#'; //현재화면으로 가게 /// 위시리스트 간헐적으로 뜨는 오류해결
			...
			$(document).on('click', '.productdetail, .info_box', function (event) {
				if($(event.target).hasClass('wishButton')){
					return;
				}
			    var productId = $(this).closest('.itemdivide').find('.productNum').text();
